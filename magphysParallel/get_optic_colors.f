...
c       Store parameters + magnitudes of each model in output file
 ...
 2	   if (aux_read.eq.0) then
	   ...
	   close (29)
	   open (29,file=infile(kfile),status='old',form='unformatted',action='read')
c          RF added action='read'
	   index=0
...
c       RF adding action='read' to allow multiple processes to read this 
	open (81,file=filtfile,form='unformatted',status='old',action='read')
